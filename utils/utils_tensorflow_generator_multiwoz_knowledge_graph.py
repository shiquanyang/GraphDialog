...
def gen_samples(data, length):
    ...
    for i in range(length):
        yield data_info['context_arr'][i],\
              ...
              data_info['ent_idx_restaurant'][i], \
              data_info['ent_idx_hotel'][i], \
              data_info['ent_idx_attraction'][i], \
              data_info['ent_idx_train'][i], \
              data_info['ent_idx_hospital'][i]
...
def get_seq(data_info, batch_size, drop_remainder):
    ds_series = tf.data.Dataset.from_generator(lambda: gen_samples(data_info, len(data_info)),
                                   output_types=(tf.int32,  # context_arr 0
                                                 ...
                                                 tf.string,  # ent_idx_restaurant 28
                                                 tf.string,  # ent_idx_hotel 29
                                                 tf.string,  # ent_idx_attraction 30
                                                 tf.string,  # ent_idx_train 31
                                                 tf.string  # ent_idx_hospital 32
                                                 ),
                                   output_shapes=((None, None),  # context_arr
                                                  ...
                                                  (None, None),  # adj
                                                  (None,),  # head_pointer
                                                  (None,),  # ent_idx_restaurant
                                                  (None,),  # ent_idx_hotel
                                                  (None,),  # ent_idx_attraction
                                                  (None,),  # ent_idx_train
                                                  (None,)  # ent_idx_hospital
                                                  )
                                   )
    print(len(data_info))
    ds_series_batch = ds_series.shuffle(len(data_info)).padded_batch(batch_size, padded_shapes=([None, MEM_TOKEN_SIZE],  # context_arr
                                                   ...
                                                   padding_values=(PAD_token,  # context_arr
                                                                   ...
                                                                   0,  # adj
                                                                   0,  # head_pointer
                                                                   'PAD',  # ent_idx_restaurant
                                                                   'PAD',  # ent_idx_hotel
                                                                   'PAD',  # ent_idx_attraction
                                                                   'PAD',  # ent_idx_train
                                                                   'PAD'  # ent_idx_hospital
                                                                   ),
