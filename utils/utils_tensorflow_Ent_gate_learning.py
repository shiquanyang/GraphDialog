...
from utils.utils_tensorflow_generator_kvr_gate_learning import *
...
def text_to_sequence(pairs, lang):
    ...
    for pair in pairs:
        ...
        sequence_data.append({
            ...
            'gate_label': tf.convert_to_tensor(pair['gate_label'])
        })
...
def prepare_data_seq(task, batch_size=100):
    ...
    context_arr, response, sketch_response, conv_arr, ptr_index, selector_index, kb_arr, context_arr_plain, response_plain,\
        ...
        ent_idx_cal, ent_idx_nav, ent_idx_wet, ent_idx_cal_lengths, ent_idx_nav_lengths, ent_idx_wet_lengths, ID, deps, deps_type, cell_masks, adj, gate_label = next(iter(train))
